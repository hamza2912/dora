...
function Footer() {
    ...
    return (
      ...
      <div className='w-full h-auto'>
          <div className='flex flex-col lg:w-2/5 w-full px-5 lg:px-0 mx-auto items-center md:pt-24 pt-16 pb-16'>
                <h2 data-aos="fade-up" className='tracking-wide'>Join the waitlist</h2>
                <h3 className='mt-5'>Get notified when we launch.</h3>
                <div className='lg:w-full w-3/4 grid md:grid-cols-3 grid-cols-1 gap-2 md:mt-16 mt-10'>
                    <input value={email} onChange={(event) => setemail(event.target.value)} className='md:col-span-2 bg-gray px-4 md:py-4 py-3 rounded-lg input-border focus:outline-none' placeholder='Email address' type="email" />
                    <button onClick={submitEmail} className='bg-brown md:py-4 py-3 text-white rounded-lg text-sm'>Join the waitlist</button>
                </div>
                <h1 className='md:text-3xl text-base tracking-wide md:mt-20 mt-12'>we are <span className='text-transparent bg-clip-text hero-text'>latina founded</span></h1>
                <p className='md:text-2xl text-base mt-2'>Say hi at hello@trydora.com</p>
                <div className='lg:w-1/3 md:w-1/4 w-1/2 grid grid-cols-3 gap-0 place-items-center md:mt-24 mt-12'>
                    <a className={isHoverInsta ? 'flex justify-center items-center rounded-full bg-insta w-10 h-10 transition-all' : 'flex justify-center items-center rounded-full bg-gray w-10 h-10 transition-all'} href="https://www.instagram.com/try.dora/" target="_blank"
                    ...
                    <a className={isHoverfb ? 'flex justify-center items-center rounded-full bg-fb w-10 h-10 transition-all' : 'flex justify-center items-center rounded-full bg-gray w-10 h-10 transition-all'} href="https://www.facebook.com/Try-Dora-100781805940266" target="_blank"
                    ...
                    onMouseOut={()=>setisHoverfb(false)}><svg width="12" height="19" viewBox="0 0 12 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                     ...
                     <a className={isHovertwitter ? 'flex justify-center items-center rounded-full bg-twitter w-10 h-10 transition-all' : 'flex justify-center items-center rounded-full bg-gray w-10 h-10 transition-all'} href="https://twitter.com/try_dora" target="_blank"
                ...
                <div>
                    ...
                    <div className='messageBox lg:w-1/3 w-4/5 bg-white h-auto z-10 fadein'>
                        ...
                        <div className='lg:w-3/4 w-4/5 flex flex-col items-center mx-auto pt-8 pb-10'>
                            ...
                            <input value={firstname} onChange={(event) => setfirstname(event.target.value)} className='lg:w-3/4 w-4/5 mt-4 bg-gray px-4 py-2 rounded-md input-border focus:outline-none' placeholder='First Name' type="text" />
                            <input value={lastname} onChange={(event) => setlastname(event.target.value)} className='lg:w-3/4 w-4/5 mt-2 bg-gray px-4 py-2 rounded-md input-border focus:outline-none' placeholder='Last Name' type="text" />
                            <input value={profession} onChange={(event) => setprofession(event.target.value)} className='lg:w-3/4 w-4/5 mt-2 bg-gray px-4 py-2 rounded-md input-border focus:outline-none' placeholder='Profession' type="text" />
                            <button onClick={submitBeta} className='w-3/4 mt-4 py-3 bg-brown text-white rounded-md text-xs'>Submit</button>
                        </div>
